...
pdf(paste(outdir, "/Multi_factor_cox_surv_curve_training.pdf", sep = ""), height = 7, width = 7)
...
#  predict(res.cox,test_gene2)
  ...
  cat(paste("Validation data survival curve P-value:", summary(coxph(Surv(time, status)~risk, data = predict_mat2))$sctest[3], "\n"), 
      file = paste(outdir, "/Muti_factor_report.txt", sep = ""), append = T)
  pdf(paste(outdir, "/Multi_factor_cox_surv_curve_validation.pdf", sep = ""), height = 7, width = 7)
  print(ggsurvplot(fit, data = predict_mat2, pval = TRUE, 
             xlab = "Time in months", conf.int = TRUE,
             risk.table.y.text.col = T, risk.table.height = 0.25, 
             risk.table.y.text = FALSE, ncensor.plot.height = 0.25,
             legend.labs = levels(as.factor(predict_mat[,3])), 
